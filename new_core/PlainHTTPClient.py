from AbstractHTTPClient import AbstractHTTPClient

import httplib
import json

class PlainHTTPClient(AbstractHTTPClient):
    """
    ...
    def sendPOST(self, data_dict, full_output=False, field=None):
        """
        ...
        if (not full_output) and type(field) == int and field < len(result):
            return result[field]
        return result

    def sendGET(self, full_output=False, field=None):
        """
