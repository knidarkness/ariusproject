...
@app.route(Extractor.getValue('flask_server_input_noise'), methods=['POST'])
def noise_input():
...
@app.route(Extractor.getValue('flask_server_output_noise'), methods=['GET'])
def noise_output():
    global noise_available
...
@app.route(Extractor.getValue('flask_server_input_client'), methods=['POST'])
...
@app.route(Extractor.getValue('flask_server_core_input_connection'), methods=['GET'])
def get_speech_text():
    global input_text
    tmp_text = input_text
    input_text = {"speech_text": "no updates"}
...
@app.route(Extractor.getValue('flask_server_core_output_connection'), methods=['POST'])
def command_input():
    global command
    print request.json
...
@app.route(Extractor.getValue('flask_server_output_client'), methods=['GET'])
def get_command():
    global command
    tmp_command = command
    print tmp_command
